...
# Load or create food data
if not os.path.exists("food_data.xlsx"):
    st.error("Food data file missing! Run 'create_food_data.py' first.")
    st.stop()
...
# Session state to store daily food log
...
if st.button("Add Foods"):
    for food, quantity in quantities.items():
        if quantity > 0:
            ...
            # Log food item with the selected date
...
if not df_log.empty:
    ...
    if "Date" in df_log.columns:
        if not df_log[df_log["Date"] == past_date].empty:
            ...
            st.write(f"### Nutrient Breakdown for {past_date}")
